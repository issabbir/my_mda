...
class FuelLimitController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('mea.vms.fuellimit.index', compact('data'));
    }
    ...
    public function store(Request $request, $id = null)
    {
        ...
        try {
            ...
            $params = [
                ...
                "p_MAIN_FUEL_ID" => $request->get("main_fuel_id"),
                ...
                "p_SINGLE_SHIFT_YN" => $request->get("single_shift_yn"),
                "p_INSERT_BY" => Auth()->ID(),
                "o_status_code" => &$statusCode,
                "o_status_message" => &$statusMessage
            ];

            DB::executeProcedure('MEA.VM_FUEL_LIMIT_PKG.fuel_limit_entry', $params);
    ...
    public function datatableList()
    {
        ...
        $querys = "select MEA.VM_FUEL_LIMIT_PKG.get_fuel_limit('" . $id . "') from dual";
        $entityList = DB::select($querys);
        return datatables()->of($entityList)
            ...
            ->addColumn('single_shift_yn', function ($query) {
                if ($query->single_shift_yn == 'Y') {
                    return 'Yes';
                } else {
                    return 'No';
                }
            ...
            ->addColumn('action', function ($query) {
                return '<a href="' . route('fuel-limit-edit', $query->fuel_limit_id) . '"><i class="bx bx-edit cursor-pointer"></i></a>';
            })->make(true);
    ...
    {
        if ($id) {
            $insertedData = $this->findInsertedData("MEA.VM_FUEL_LIMIT_PKG.get_fuel_limit", $id);
        }
        ...
        $data = [
            ...
            'get_fuel_types' => $this->commonDropDownLookupsList('vm_lookup_pkg.get_fuel_types', $insertedData->main_fuel_id),
    ...
    public function update(Request $request, $id)
    {
        ...
        $flashMessageContent = $this->flashMessageManager->getMessage($params);
        return redirect()->route('fuel-limit-index')->with($flashMessageContent['class'], $flashMessageContent['message']);
    }
