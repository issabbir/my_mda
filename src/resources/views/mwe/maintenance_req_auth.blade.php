...
@section('title')
    Maintenance Request Authorization
@endsection
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <h4 class="card-title">Maintenance Request Authorization</h4>
                        <form method="POST" action="" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                                    ...
                                                    <li class="d-inline-block mr-2 mb-1">
                                                        <fieldset
                                                            ...
                                                            <div class="custom-control custom-radio">
                                                                <input type="radio" class="custom-control-input"
                                                                       value="{{ old('status','3') }}"
                                                                       {{isset($data->status) && $data->status == "3" ? 'checked' : ''}}
                                                                       name="status"
                                                                       id="customRadio1">
                                ...
                                @endif
                                {{--                                @if(in_array($data->status,App\Enums\Mwe\ConfigRole::can_be_assign_inspector))--}}

                                <div class="col-md-4 approv">
                                    <div class="row my-1">
                                        <div class="col-md-12">
                                            <label class="input-required">Assign Senior Sub-Assistant Engineer</label>
                                            <select style="width: 100%" class="form-control select2" disabled>
                                                <option value="">MOHAMMED AFSAR UDDIN - 000504</option>
                                            </select>
                                            <input type="hidden" name="inspector_emp_id" value="2010300103334088">
                                            {{--<select style="width: 100%" name="inspector_emp_id" id="inspector_emp_id"
                                                    class="form-control select2"{{isset($data->status) && $data->status == "12" ? 'disabled' : ''}}>
                                                @if(isset($assigned_inspector->emp_id))
                                                    <option value="{{$assigned_inspector->emp_id}}"
                                                            selected>{{isset($assigned_inspector->emp_name) ? $assigned_inspector->emp_name : ''}}</option>
                                                @endif
                                                <option value="">Select one</option>
                                            </select>--}}
                                            @if ($errors->has('inspector_emp_id'))
                                                <span class="help-block">{{ $errors->first('inspector_emp_id') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="row my-1">
                                        <div class="col-md-12">
                                            <label>Approval Status of Deputy Chief Engineer</label>
                                            <input type="text" name="deputy_chief_eng_app_status"
                                                   id="deputy_chief_eng_app_status" readonly
                                                   {{--value="{{($data->deputy_chief_eng_app_status=='Y')?'Approved':'Rejected'}}"--}}
                                                   value="@if($data->deputy_chief_eng_app_status!='')@if($data->deputy_chief_eng_app_status=='Y') Approved @else Rejected @endif @endif"
                                                   class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="row my-1">
                                        <div class="col-md-12">
                                            <label>Name Of Deputy Chief Engineer</label>
                                            <input type="text" name="name_of_deputy_engineer"
                                                   id="name_of_deputy_engineer" readonly
                                                   value="{{isset($deputy_engineer)?$deputy_engineer->emp_name.' - '.$deputy_engineer->emp_code:''}}"
                                                   class="form-control">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="row my-1">
                                        <div class="col-md-12">
                                            ...
                                            <textarea type="text" name="remarks_xen" placeholder="Remarks"
                                                      class="form-control" @if(isset($data->remarks_xen)) readonly @endif
                                                      oninput="this.value = this.value.toUpperCase()">{{ old('remarks_xen', $data->remarks_xen) }}</textarea>
